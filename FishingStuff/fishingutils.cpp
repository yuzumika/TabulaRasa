...
#pragma region Calculations
    ...
    ************************************************************************/
    ...
    float GetHourlyModifier(fish_t* fish) { // 0.25 to 1.25
        float modifier = 0.5f;
        uint8 hour = (uint8)CVanaTime::getInstance()->getHour();
...
#pragma region Fishing
    ...
    ************************************************************************/
    ...
    fishresponse_t *FishingCheck(CCharEntity* PChar, uint8 fishingSkill, rod_t* rod, bait_t* bait, fishingarea_t* area)
    {
        ...
        if (FishPool.size()) {
            ...
            for (auto fish : FishPool) {
                ...
                if ((fishingSkill >= fishIter->maxSkill || fishIter->maxSkill - fishingSkill <= 100) &&         // Skill diff okay                                                                                                   
                    (!fishIter->reqKeyItem || charutils::hasKeyItem(PChar, fishIter->reqKeyItem))) {            // Key item okay
        ...
        if (ItemPool.size()) {
            for (auto item : ItemPool) {
                if (RemoveList.count(item->fishID) > 0)
                    continue;
                if (item->quest_only || !item->reqKeyItem || charutils::hasKeyItem(PChar, item->reqKeyItem)) {  // Key item okay
        ...
        if (FishSelection != nullptr && selector < FishPoolWeight) {                                            // Hooked fish
        ...
        else if (ItemSelection != nullptr && selector < ItemPoolWeight + FishPoolWeight) {                              // Hooked item
            response->hooked          = true;
        ...
        else if (MobSelection != nullptr && selector < ItemPoolWeight + FishPoolWeight + MobPoolWeight) {               // Hooked mob
        ...
        else if (ChestSelection > 0 && selector < ItemPoolWeight + FishPoolWeight + MobPoolWeight + ChestPoolWeight) {        // Hooked chest
            response->hooked          = true;
